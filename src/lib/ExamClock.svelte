<script lang="ts">
	...
	const DEFAULT_NEXT_CHECKPOINT_COLOR = '#4F46E5';
	const NEXT_GRAY_BG_COLOR = 'rgb(229, 231, 235)';
	...
	$: nextProgressBgColor = nextCheckpoint?.color ?? DEFAULT_NEXT_CHECKPOINT_COLOR;
	...
	$: nextTextColorOnGray = highContrastMode
		? '#000000'
		: getTextColorForBackground(NEXT_GRAY_BG_COLOR, '#000000');

	// Determine effective text color based on progress percentage and position
	$: nextLeftSideTextColor =
		nextCheckpointProgress > 30 ? nextTextColorOnProgress : nextTextColorOnGray;
	$: nextRightSideTextColor =
		nextCheckpointProgress > 85 ? nextTextColorOnProgress : nextTextColorOnGray;
	...
	const dispatch = createEventDispatcher<{ toggleTimeFormat: void }>();
...
<div class="w-full text-center">
	...
	>
		...
		<div
			...
			class="mb-0 w-full overflow-visible px-2 text-center font-mono leading-[0.9] font-bold transition-all duration-500 ease-out"
		...
		{#if showDate || showTimezone}
			<div class="mb-1 flex items-center justify-center space-x-3">
				{#if showDate}
					<div
						class="text-xl font-medium transition-all duration-300 ease-out md:text-2xl lg:text-3xl {highContrastMode
				...
				{#if showTimezone && timezone}
					<div
						class="text-lg transition-all duration-300 ease-out md:text-xl lg:text-2xl {highContrastMode
		...
		{#if nextCheckpoint}
			<div
				class="relative mx-auto mt-6 max-w-md overflow-hidden rounded-lg transition-all duration-400 ease-out"
				style="border: {highContrastMode ? '2px solid white' : `2px solid ${nextCheckpoint.color}`};
					box-shadow: {highContrastMode
					? `0 0 10px ${nextCheckpoint.color}, 0 0 15px ${nextCheckpoint.color}`
					: 'none'};"
			...
			>
				...
				<div class="relative z-10 flex w-full items-center justify-between p-4">
					<div class="flex items-center">
						<span class="mr-3 text-2xl transition-transform duration-200"
							>{nextCheckpoint.emoji}</span
						>
						<div class="text-base md:text-lg">
							...
							<span
								class="ml-2 {highContrastMode ? 'font-bold' : ''}"
								style="color: {highContrastMode ? 'black' : nextLeftSideTextColor}"
							>
	...
	<div
		class="mt-4 flex items-center justify-center space-x-4 text-base transition-all duration-300 ease-out md:text-lg {highContrastMode
			? 'text-white'
			: 'text-gray-500'}"
	>
		<div class="flex items-center space-x-2">
			<span class="h-3 w-3 animate-pulse rounded-full bg-green-500"></span>
			<span class={highContrastMode ? 'font-bold' : ''}
				>Live clock • Syncs with server every 5 minutes</span
			>
		</div>
		<button
			on:click={toggleTimeFormat}
			class="transform rounded-lg border px-4 py-2 text-base font-medium transition-all duration-200 ease-out hover:scale-105 active:scale-95 {highContrastMode
				? 'border-2 border-white bg-yellow-400 text-black hover:bg-yellow-300 hover:shadow-lg'
